...
export default function HomePage() {
  ...
  const addToCart = (book: any, e: React.MouseEvent) => {
    ...
    if (!exists) {
      ...
      toast.error(`${book.name} allaqachon savatda mavjud`, {
        position: "top-center",
        duration: 2500,
      })
    }
  ...
  const newBooks = books.filter((book) => isBookNew(book.createdAt)).slice(0, 5)
...
>
  <CardContent className="p-4 flex-grow flex flex-col">
    ...
    <div className="space-y-1 text-sm text-muted-foreground mb-4">
      <p>{book.page} bet</p>
      <p>{book.year}-yil</p>
      <p className="text-xs text-[#21466D]">Muallif: {book.Auther?.name || "Nomaʼlum"}</p>
    </div>
  </CardContent>
