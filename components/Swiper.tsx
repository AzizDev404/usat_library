...
export default function BookSwiper({ initialBooks, initialBookItems }: BookSwiperProps) {
  ...
  {book.name.length > 20 ? `${book.name.slice(0, 20)}...` : book.name}
                    ...
                    <p className="text-sm text-[#21466D]/70">Muallif: {book.Auther?.name || "Noma'lum"}</p>
                    ...
                    {book.bookItem?.PDFFile && (
                      <div className="text-xs text-green-600 bg-green-50 px-2 py-1 rounded">
                        PDF mavjud ({Math.round(book.bookItem.PDFFile.file_size / 1024)} KB)
                      </div>
                    )}
            ...
            >
              {enrichedBooks.map((book, index) => (
                <SwiperSlide key={`desktop-${book.id}-${index}`}>
                  <div className="flex items-center justify-center px-4 h-[650px]">
                    <div className="bg-white/95 shadow-sm dark:bg-gray-800/95 backdrop-blur-lg rounded-3xl p-6 border border-[#21466D]/10 w-full max-w-6xl h-full  transform transition-all duration-500  hover:scale-[1]">
  ...
  {book.name.length > 20 ? `${book.name.slice(0, 20)}...` : book.name}
                            ...
                            <p className="text-lg text-[#21466D]/80 dark:text-white/80 mb-4">
                              Muallif: {book.Auther?.name || "Noma'lum"}
                            </p>
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            <div className="bg-gradient-to-br from-[#21466D]/10 to-[#21466D]/5 rounded-xl p-4 text-center">
                              <p className="text-[#21466D]/60 font-medium text-sm mb-1">Sahifalar</p>
                            ...
                            <div className="bg-gradient-to-br from-[#ffc82a]/20 to-[#ffc82a]/10 rounded-xl p-4 text-center">
                              <p className="text-[#21466D]/60 font-medium text-sm mb-1">Yil</p>
                              <p className="text-[#21466D] dark:text-white font-bold text-lg">{book.year}</p>
                            </div>
                            <div className="bg-gradient-to-br from-[#21466D]/10 to-[#21466D]/5 rounded-xl p-4 text-center">
                              <p className="text-[#21466D]/60 font-medium text-sm mb-1">Nusxalar</p>
            ...
            {/* Navigation buttons */}
            <div className="swiper-button-prev !absolute !left-8 !top-1/2 !-translate-y-1/2 !z-20 !bg-white/90 backdrop-blur-sm !border-2 !border-[#21466D]/20 !text-[#21466D] hover:!bg-[#21466D] hover:!text-white !w-14 !h-14 !rounded-full !flex !justify-center !items-center !transition-all !duration-300 !shadow-lg hover:!shadow-xl after:!text-xl after:!font-bold"></div>
            <div className="swiper-button-next !absolute !right-8 !top-1/2 !-translate-y-1/2 !z-20 !bg-white/90 backdrop-blur-sm !border-2 !border-[#21466D]/20 !text-[#21466D] hover:!bg-[#21466D] hover:!text-white !w-14 !h-14 !rounded-full !flex !justify-center !items-center !transition-all !duration-300 !shadow-lg hover:!shadow-xl after:!text-xl after:!font-bold"></div>
          </div>
        </div>
        </div>
      )}
